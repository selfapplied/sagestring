...
class SVGKernelSystem {
  constructor(options = {}) {
    ...
    this.matcher = new KernelMatcher();
  ...
  evolveLattice(kernelId, inputLattice = null) {
    ...
    const newLattice = this.matcher.evolve(kernel, currentLattice, size);
  ...
  /**
   * Compute kernel match (delegates to matcher)
   */
  computeKernelMatch(kernel, lattice, x, y, size) {
    return this.matcher.match(kernel, lattice, x, y, size);
  }

  /**
   * Compute coupling sum (delegates to matcher)
   */
  computeCouplingSum(kernel, lattice, x, y, size) {
    return this.matcher.coupling(lattice, x, y, size);
  }
